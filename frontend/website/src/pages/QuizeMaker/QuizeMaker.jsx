...
export default function QuizeMaker() {
  ...
  // Check for section ID in query params (for automatically created quizzes)
  useEffect(() => {
  ...
  const saveAnswer = async (questionId, selectedAnswer) => {
    try {
      ...
      await axios.post("http://localhost:4545/quiz-submission/answer", {
        submissionId,
        questionId,
        selectedAnswer
      }, {
  ...
  const handleDeleteQuestion = async (questionId, questionIndex) => {
    try {
      // If the question has an ID, delete it from the database
      if (questionId) {
        await axios.delete(`http://localhost:4545/question/delete/${questionId}`, {
          headers: {
            token: localStorage.getItem("token")
          }
        });
      ...
      // Update the quiz info to reflect the new total marks
      setQuizInfo(prev => ({
        ...prev,
  ...
  const checkQuizExists = async () => {
    try {
      ...
      // If we have a section ID, use the new endpoint to get quiz by section
      let data;
      if (sectionId) {
        const response = await axios.get(`http://localhost:4545/quiz/section/${sectionId}`, {
          headers: {
            token: localStorage.getItem("token")
          }
        });
        ...
        // Update quiz info from the response
        if (data.quiz) {
          setQuizInfo({
            id: data.quiz.id,
            title: data.quiz.title || `Quiz #${data.quiz.id}`,
            description: data.quiz.description || "Edit this quiz to add a description",
            totalMarks: data.quiz.totalMarks || 0,
            difficulty: data.quiz.difficulty || 'medium',
            timeLimit: data.quiz.timeLimit || data.quiz.duration || 30 // Use timeLimit or duration field, default to 30 min
          });
        ...
        // Use the regular questions endpoint to check if the quiz exists
        const response = await axios.get(`http://localhost:4545/question/getall/${quizId}`, {
          headers: {
            token: localStorage.getItem("token")
          }
        });
