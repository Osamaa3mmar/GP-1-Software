...
export default function CourseCardAdmin({courses}) {
  return (
    ...
    {courses?
courses.map((course)=>{
    return (<Grid key={course.id} sx={{boxShadow:"0px 0px 6px rgba(0,0,0,0.4)",borderRadius:"12px",padding:"10px",overflow:"hidden"}} size={{lg:4,md:5,sm:6,xs:10}}>
    <CardImage url={course.thumbnail}/>
    <Box>
    <Stack sx={{padding:"10px 20px 15px 20px"}} >
        <Box sx={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <h2 style={{fontWeight:"600",fontSize:"22px"}}>{course.title}</h2>
        <Button sx={{color:"#6366F1" ,bgcolor:"#6366F110"}}>View</Button>
        </Box>
        <Stack direction={"row"} sx={{width:"100%",display:"flex" ,justifyContent:"space-between",padding:"15px"}}>
            ...
            <Stack spacing={2} direction={"column"} sx={{flexGrow:1,display:"flex",justifyContent:"center" ,alignItems:"center"}}>
             ...
             <SwitchActive state={course.completionStatus}/>
            </Stack>
        </Stack>
        {/* <Stack>
